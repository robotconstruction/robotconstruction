...
double WALK_ACC         = 5;
double WALK_EXTENDED_X  = 16;      // this is to control the speed of the robot, did not been modified in this file
double WALK_EXTENDED_Z  = 25;      //  control the extend in y axis, do not been modified also
double WALK_SIDE_MAX    = 30;
double WALK_MASS_ADJUST = 0;
...
void autosingleGaitCtrl(uint8_t LegNum, uint8_t statusInput, float cycleInput, float directionInput){
  double rDist;
  double xGait;
  double yGait;
  double zGait;
  ...
  if(cycleInput < (1 - WALK_LIFT_PROP))
  {
    ...
    else if(cycleInput > (WALK_ACC/(WALK_ACC*2 + WALK_RANGE*statusInput))*(1-WALK_LIFT_PROP) && cycleInput <= ((WALK_ACC + WALK_RANGE*statusInput)/(WALK_ACC*2 + WALK_RANGE*statusInput))*(1-WALK_LIFT_PROP))
    {
      ...
      roll = roll * kr;
      //adjust Y
      if(LegNum == 1){ybuffer  = WALK_HEIGHT + pitch + roll;}
      else if(LegNum == 2){ybuffer  = WALK_HEIGHT + pitch - roll;}
      else if(LegNum == 3){ybuffer  = WALK_HEIGHT - pitch + roll;}
      else if(LegNum == 4){ybuffer  = WALK_HEIGHT - pitch - roll;}
      if(ybuffer > WALK_HEIGHT_MAX){ybuffer = WALK_HEIGHT_MAX;}
      if(ybuffer < WALK_HEIGHT_MIN){ybuffer = WALK_HEIGHT_MIN;}
      yGait = ybuffer;

      //adjust Z
      if(LegNum == 1){zbuffer = -roll;}
      else if(LegNum == 2){zbuffer = -roll;}
      else if(LegNum == 3){zbuffer = roll;}
      else if(LegNum == 4){zbuffer = roll;}
...
////////////////////////////////////////////////////////////calculate gesture end //////////////////////
  ...
  if(!turnCmd){
    autosingleGaitCtrl(1, 1, StepA,  directionAngle);
    autosingleGaitCtrl(3, 1, StepB,  directionAngle);
    autosingleGaitCtrl(4, 1, StepC, -directionAngle);
    autosingleGaitCtrl(2, 1, StepD, -directionAngle);
  }
...
// add the auuto gait as function D
